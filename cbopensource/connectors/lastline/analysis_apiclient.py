...
class AnalysisClientBase(object):
    ...
    def get_scanner_hits(self, scanner, after=None, before=None,
                         min_version=0, max_version=None, test_flag=None,
                         raw=False):
        """
        Find tasks that triggered a certain scanner.

        :param scanner: Name of the scanner.
        :param after: Find tasks completed after this time.
        :param before: Find tasks completed before this time.
        :param min_version: Minimum version of scanner to find.
        :param max_version: Maximum version of scanner to find.
        :param test_flag: If True, only include backend-tasks where the scanner
            was in *test* mode; if False, only include backend-tasks where the
            scanner was in *real* mode; otherwise include any backend-tasks
            regardless of the *test* flag.
        """
        if before and hasattr(before, "strftime"):
            before = before.strftime(AnalysisClientBase.DATETIME_FMT)
        if after and hasattr(after, "strftime"):
            after = after.strftime(AnalysisClientBase.DATETIME_FMT)

        url = self._build_url('research', [ 'get_scanner_hits' ])
        params = purge_none({
            'scanner': scanner,
            'after': after,
            'before': before,
    ...
    def get_task_status(self, uuid, raw=False):
        """
        Check the (internal) status of a task.

        :param uuid: the unique identifier of the submitted task,
            as returned in the task_uuid field of submit methods.
        """
        url = self._build_url('research', [ 'get_task_status' ])
        params = { 'uuid': uuid }
        return self._api_request(url, params, raw=raw, post=True)
    ...
    def get_detections(self, report_uuid, raw=False):
        ...
        """
        url = self._build_url('research', [ 'get_detections' ])
    ...
    def get_backend_scores(self, md5=None, sha1=None, raw=False):
        ...
        url = self._build_url('research', [ 'get_backend_scores' ])
        params = purge_none({
            'file_md5': md5,
            'file_sha1': sha1,
        })
